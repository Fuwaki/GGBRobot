...
bool send_image(const cv::Mat &img)
{
    ...
    if (!send_data(client_socket, &width, sizeof(width)))
        return false;
    if (!send_data(client_socket, &height, sizeof(height)))
        return false;
    if (!send_data(client_socket, &channels, sizeof(channels)))
        return false;
    if (!send_data(client_socket, img.data, len))
        return false;
...
bool send_detection_result(const ImageDetector::Circle &result)
{
    ...
    // 手动序列化以避免结构体填充问题
    if (!send_data(client_socket, &result.center, sizeof(result.center)))
        return false;
    if (!send_data(client_socket, &result.radius, sizeof(result.radius)))
        return false;
    if (!send_data(client_socket, &result.found, sizeof(result.found)))
        return false;
    return true;
}
