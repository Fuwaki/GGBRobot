...
void run_matrix_benchmark()
{
    ...
    ESP_LOGI(TAG, "--- 测试: 求逆 (4x4) ---");
    {
        ...
        ESP_LOGI(TAG, "[dspm::Mat]       平均每次迭代: %.2f us (成功率: %d/%d)", (float)total_time_us / ITERATIONS,
                 count, ITERATIONS);
    }
    {
        cv::Mat a(4, 4, CV_32F);
        ...
        ESP_LOGI(TAG, "[OpenCV]           平均每次迭代: %.2f us (成功率: %d/%d)", (float)total_time_us / ITERATIONS,
                 count, ITERATIONS);
    }
