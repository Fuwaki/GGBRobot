...
ServoAngles move_platform(float pitch, float roll, float h)
{
    ...
    // 俯仰(Pitch)旋转矩阵 (绕Y轴)
    Mat pitch_rot(3, 3);
    pitch_rot(0, 0) = cosf(pitch);
    pitch_rot(0, 1) = 0;
    pitch_rot(0, 2) = sinf(pitch);
    pitch_rot(1, 0) = 0;
    pitch_rot(1, 1) = 1;
    pitch_rot(1, 2) = 0;
    pitch_rot(2, 0) = -sinf(pitch);
    pitch_rot(2, 1) = 0;
    pitch_rot(2, 2) = cosf(pitch);
    ...
    roll_rot(0, 2) = 0;
    roll_rot(1, 0) = 0;
    roll_rot(1, 1) = cosf(roll);
    roll_rot(1, 2) = -sinf(roll);
    roll_rot(2, 0) = 0;
    roll_rot(2, 1) = sinf(roll);
    roll_rot(2, 2) = cosf(roll);
