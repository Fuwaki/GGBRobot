...
void Robot::update_pid_controllers(const ImageDetector::Circle& ball, const cv::Size& frame_size) {
    if (ball.found) {
        ...
        // 使用低通滤波器平滑误差,减少抖动
        filtered_x_error_ = Params::LOW_PASS_FILTER_ALPHA * x_error + (1 - Params::LOW_PASS_FILTER_ALPHA) * filtered_x_error_;
        filtered_y_error_ = Params::LOW_PASS_FILTER_ALPHA * y_error + (1 - Params::LOW_PASS_FILTER_ALPHA) * filtered_y_error_;
...
void Robot::control_loop_iteration() {
    ...
    KinematicsSolver::ServoAngles angles = KinematicsSolver::move_platform(
        y_controller_->get_adjustment(), 
        x_controller_->get_adjustment(), 
        Params::KINEMATICS_DEFAULT_HEIGHT
    );
